...
d_final2 %>%
  dplyr::mutate(index_fname = glue::glue("{fname}.crai")) %>%
  dplyr::select(sample_name, fname, index_fname) %>%
  tidyr::pivot_longer(fname:index_fname, values_to = "cram_or_index_fname") %>%
  dplyr::arrange(sample_name) %>%
  dplyr::select(cram_or_index_fname) %>%
  readr::write_csv("data/filtered65.csv", col_names = FALSE)
